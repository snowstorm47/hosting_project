...
import { fetchSingleInstractor, resetInstractor } from "../../store/instractor/actions";
import { baseURL } from "../../utils/axios";

const InstractorDetail = () => {
  const params = useParams();
  const dispatch = useDispatch();
  const { loading, data, error } = useSelector((state) => state.fetchInstractor);

  useEffect(() => {
    const id = params.id;
    dispatch(fetchSingleInstractor(id));
  }, []);
  return (
    <div className="p-7">
      <Modal open={error}>
        <Dialog
          severity="failure"
          message="fetching instractor failed !"
          close={() => dispatch(resetInstractor())}
        />
      </Modal>
      <Modal open={loading}>
            ...
            <div className="p-7">
              ...
              <div className="grid grid-cols-2 mt-2">
                {data.educations.map((education) => {
                  return (
                    <div className="space-y-1 shadow-xl p-2 rounded-lg">
                      <div className="flex space-x-2">
                        <label className="font-semibold capitalize">
                          Certificate Id :
                        </label>
                      ...
                      <div className="flex space-x-2">
                        <button onClick={()=>{
                            window.open(`${baseURL}/images/${education.educational_document}`, '_blank')
